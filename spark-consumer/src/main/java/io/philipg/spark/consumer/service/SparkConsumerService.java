...
public class SparkConsumerService {
    ...
    public void run(){
        ...
        NaiveBayesModel model = NaiveBayesModel.load(jssc.sparkContext().sc(), "src/main/resources/myNaiveBayesModel");
        ...
        lines
                ...
                .foreachRDD(rrdd -> {
                    ...
                    sorted.forEach( record -> {
                        System.out.println(String.format(" %s (%d)", model.predict(tf.transform(Arrays.asList(record._2.split(" ")))), record._1));
                    });
                });
